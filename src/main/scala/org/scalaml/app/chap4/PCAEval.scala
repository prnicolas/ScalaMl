...
object PCAEval extends UnsupervisedLearningEval {
  ...
  override def run(args: Array[String]): Unit = {
	  ...
	  pca |> timeSeries match {
	  	case Some(covariance) => {
	  		covariance._2.foreach( println )
	  		covariance._1.foreach( r => println(r.foldLeft(new StringBuilder)((b, c) => b.append(c).append(",")).toString))
	  	}
	  	case None => println("PCA failed to extract principal components")
	  }
  }
